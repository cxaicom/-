...
{   
    ...
    Vector2f p;
    p << x, y;
    Vector2f ac = _v[2].head(2) - _v[0].head(2);//向量AC
    Vector2f cb = _v[1].head(2) - _v[2].head(2);//向量CB
    Vector2f ba = _v[0].head(2) - _v[1].head(2);//向量BA
    Vector2f ap, bp, cp;
    ap = p - _v[0].head(2);
    bp = p - _v[1].head(2);
    cp = p - _v[2].head(2);
...
void rst::rasterizer::rasterize_triangle(const Triangle& t) {
    auto v = t.toVector4();//返回值是array<Vector4f,3>
